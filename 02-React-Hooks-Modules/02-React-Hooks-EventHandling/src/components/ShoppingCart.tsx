...
let ShoppingCart: React.FC<IProps> = () => {
  ...
  let incrQty = (sno: string) => {
    let incrProducts: IProduct[] = productState.products.map((product) => {
      if (product.sno === sno) {
        return {
          ...product,
          qty: product.qty + 1,
  ...
  let decrQty = (sno: string) => {
    let decrProducts: IProduct[] = productState.products.map((product) => {
      if (product.sno === sno) {
        return {
          ...product,
          ...
          <div className="row">
            <div className="col">
              ...
              <p>
                ...
                officia ab, dicta quisquam odio esse deleniti quas quidem cumque
                molestiae reprehenderit expedita, aliquid nam temporibus
                doloremque vel quia minima! Animi at, nesciunt eum tempora
                omnis, sed a commodi officiis esse voluptates natus odio soluta!
                Ad reiciendis voluptate optio aperiam!
              </p>
            </div>
          </div>
        </div>
